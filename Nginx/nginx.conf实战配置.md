...
default_type  application/octet-stream;
    ...
    server {
        ...
        server_name _;
        location / {
            proxy_pass http://localhost:8001;     # 转发到本机的8001端口
    ...
    server {
        ...
        location / {
            ...
            proxy_pass http://localhost:8002;     # 转发到本机的8002端口
            proxy_http_version 1.1;
            proxy_set_header  Upgrade $http_upgrade;
            proxy_set_header  Connection keep-alive;
            proxy_cache_bypass $http_upgrade;
        ...
        # 前端静态项目
    server {
        listen 83 default_server;    # 监听83端口
        listen [::]:83 default_server;
        server_name _;
        root /Users/lidongqiang/DjangoProjects/autotest_frontend/dist;   # 前端打包后的目录
        index index.html;
        location / {
        }
